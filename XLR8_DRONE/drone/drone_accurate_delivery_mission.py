...
def geocord(distance):
#put your geo coordinates with the distance to find the new geocordinates(our aim)
    coord=0
    heading_angle=0
    return coord,heading_angle
...
for i in range(2):
    ...
    while True:
        ...
        ##Calculate distance 
            distance,heading_angle=calculate_distance(centerpt)
        #Using heading angle and the distance calculated find the new geo cordinates
            geocoord=geocord(distance,heading_angle)
            go_to_required_location(geocoord)
